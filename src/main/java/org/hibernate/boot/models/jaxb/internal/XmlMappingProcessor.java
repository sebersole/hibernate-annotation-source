...
import org.hibernate.boot.models.bind.internal.global.TypeContributionProcessor;
...
import org.hibernate.boot.models.source.internal.AnnotationAttributeValueImpl;
import org.hibernate.boot.models.source.internal.AnnotationUsageImpl;
import org.hibernate.boot.models.source.internal.dynamic.ClassDetailsImpl;
import org.hibernate.boot.models.source.internal.dynamic.DynamicMemberDetails;
import org.hibernate.boot.models.source.internal.dynamic.FieldDetailsImpl;
import org.hibernate.boot.models.source.internal.dynamic.MethodDetailsImpl;
...
public class XmlMappingProcessor {
	...
	private final Map<String, ClassProcessingContextImpl> classBindingMap = new HashMap<>();

	private XmlMappingProcessor(ModelProcessingContext processingContext) {
	...
	private void processMappingXml(
		...
		final DocumentProcessingContextImpl documentContext = new DocumentProcessingContextImpl(
				root,
				binding.getOrigin(),
				processingContext
		);
	...
	private void processGlobals(JaxbEntityMappings root, DocumentProcessingContext localContext) {
	...
	private void processConverters(List<JaxbConverter> converters, DocumentProcessingContext localContext) {
	...
	private void processSequenceGenerators(
			...
			DocumentProcessingContext localContext) {
	...
	private void prepareClassBindings(
			...
			DocumentProcessingContextImpl documentContext) {
	...
	private void processClassBindings() {
		classBindingMap.forEach( (name, classProcessingContextImpl) -> {
			// todo (annotation-source) : more overly simplistic stuff
			if ( classProcessingContextImpl.getClassNode() instanceof JaxbEntity ) {
				processEntity( (JaxbEntity) classProcessingContextImpl.getClassNode(), classProcessingContextImpl );
	...
	private void processEntity(JaxbEntity classNode, ClassProcessingContext processingContext) {
	...
	private void createCompleteEntity(JaxbEntity classNode, ClassProcessingContext processingContext) {
	...
	private void applyEntityAnnotation(
			...
			String name,
			ClassProcessingContext processingContext) {
		...
		if ( isNotEmpty( classNode.getTableExpression() ) ) {
			...
			if ( isNotEmpty( classNode.getTable().getName() ) ) {
				tableAttributes.add( new AnnotationAttributeValueImpl<>(
						...
						StringHelper.nullIfEmpty( classNode.getTable().getName() )
				) );
			}
			...
			if ( isNotEmpty( classNode.getTable().getComment() ) ) {
				classDetails.apply( new AnnotationUsageImpl<>(
						...
						classDetails,
						singletonList( new AnnotationAttributeValueImpl<>(
								HibernateAnnotations.COMMENT.getAttribute( VALUE ),
								classNode.getTable().getCheck()
						) )
				) );
			}
	...
	private void applySecondaryTableAnnotations(ClassDetailsImpl classDetails, JaxbEntity classNode, ClassProcessingContext processingContext) {
	...
	private static DynamicMemberDetails resolveAttributeMember(
		...
		if ( accessType == AccessType.FIELD ) {
			final FieldDetailsImpl fieldDetails = new FieldDetailsImpl( attrNode.getName(), processingContext.getModelProcessingContext() );
			...
			final MethodDetailsImpl methodDetails = new MethodDetailsImpl( null, attrNode.getName(), processingContext.getModelProcessingContext() );
	...
	private void processCompleteBasicAttribute(DynamicMemberDetails attributeMember, JaxbBasic jaxbBasic) {
		...
		if ( jaxbBasic.getEnumerated() != null ) {
			attributeMember.apply( new AnnotationUsageImpl<>(
					...
					attributeMember,
					singletonList( new AnnotationAttributeValueImpl<>(
							JpaAnnotations.ENUMERATED.getAttribute( VALUE ),
	...
	private void processPersistentAttribute(DynamicMemberDetails attributeMember, PersistentAttribute jaxbNode) {
		if ( jaxbNode.getAccess() != null ) {
			attributeMember.apply( new AnnotationUsageImpl<>(
					...
					attributeMember,
					singletonList(
							new AnnotationAttributeValueImpl<>( JpaAnnotations.ACCESS.getAttribute(VALUE), jaxbNode.getAccess() )
					)
			) );
		}
